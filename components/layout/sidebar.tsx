...
export function Sidebar({
  ...
  return (
    <>
      ...
      <motion.aside
        animate={{
          x: isDesktop ? 0 : open ? 0 : -336,
        }}
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex w-[296px] flex-col bg-[linear-gradient(180deg,rgba(242,247,252,0.99),rgba(236,242,248,0.99))] px-4 py-4 shadow-[18px_0_40px_rgba(15,23,42,0.06)] md:sticky md:inset-y-auto md:top-0 md:h-screen md:self-start md:overflow-y-auto md:shadow-none",
        ...
        >
          ...
          >
            <Link
              ...
              className={cn(
                ...
                isCompact && "md:justify-center md:gap-0 md:px-0 md:bg-transparent md:shadow-none",
              )}
              onClick={onClose}
              title={BRANDING.shortName}
            >
              <span className="flex h-11 w-11 shrink-0 flex-none items-center justify-center rounded-[1.15rem] bg-[linear-gradient(135deg,var(--color-primary),var(--color-accent))] text-white shadow-[0_14px_28px_rgba(30,58,95,0.24)]">
              ...
              {showLabels ? (
                ...
                <span className="block truncate text-[1.02rem] leading-tight text-slate-950">
          ...
          <div
            className={cn(
              ...
              isCompact && "md:bg-transparent md:shadow-none md:p-0",
            )}
          >
            <LayoutGroup id="sidebar-navigation">
              <nav className={cn("space-y-1.5", isCompact && "md:mt-0")}>
                {items.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      href={item.href}
                      className={cn(
                        "group relative flex items-center gap-3 overflow-hidden rounded-[1.2rem] px-3 py-3 text-sm font-medium",
                    ...
                    >
                      {active ? (
                        <>
                          <motion.span
                            className="absolute inset-0 rounded-[1.2rem] border border-white/10 bg-[linear-gradient(135deg,#16314f,#1e3a5f)]"
                            layoutId="sidebar-active-pill"
                            transition={mobileSidebarTransition}
                          />
                          <motion.span
                            animate={{ opacity: 1, scale: 1 }}
                            className="absolute -right-5 top-1/2 h-14 w-14 -translate-y-1/2 rounded-full bg-white/10 blur-sm"
                      ...
                      <span
                        className={cn(
                          "relative z-10 flex h-9 w-9 shrink-0 flex-none items-center justify-center rounded-[0.95rem] transition-colors duration-200",
                          active
